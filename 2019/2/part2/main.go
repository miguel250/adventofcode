...
func execExpected(input io.Reader, excepted int) (string, error) {
	currentZeroValue := 0
	...
	for firstParameter := 0; excepted != currentZeroValue && firstParameter < 99; firstParameter++ {
		for secondParameter := 0; excepted != currentZeroValue && secondParameter < 99; secondParameter++ {
			p.reset()
			p.store(firstParameter, 1)
			p.store(secondParameter, 2)
			err = p.eval()
			...
			currentZeroValue, err = p.getValue(0)
			if err != nil {
				return "", err
			}
		}
...
func main() {
	...
	expectedMemoryValuePtr := flag.Int("excepted", 0, "Calculate possible parements for possible 1 and 2")
	flag.Parse()
	...
	if *expectedMemoryValuePtr > 0 {
		result, err = execExpected(input, *expectedMemoryValuePtr)
	} else {
