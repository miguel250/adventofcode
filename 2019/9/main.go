...
func (p *program) parser(input io.Reader, increaseMemory bool) error {
	scanner := bufio.NewScanner(input)
	scanner.Split(splitOnComma)

	if increaseMemory {
		p.opsCodes = make([]int, maxMemory)
	}
	memoryPosition := 0
	...
	for scanner.Scan() {
		...
		if increaseMemory {
			p.opsCodes[memoryPosition] = opsCode
			memoryPosition++
		}
...
func (p *program) eval(inputValue []int) error {
	...
	for p.hasNext() {
		...
		case adjustRelativeBase:
			...
			p.relativeModeBase = p.relativeModeBase + value
		default:
...
func (p *program) getValue(position int) (int, error) {
	...
	if mode == relativeMode {
		position = p.relativeModeBase + position
	}
...
func main() {
	...
	increaseMemoryPtr := flag.Bool("increase-memory", false, "increase memory usaged")
	flag.Parse()
	...
	if !*sequencePtr && !*generatePtr {
		result, output, execErr = exec(input, inputsInt, *increaseMemoryPtr)
	}

	if *sequencePtr {
		p := &program{}
		err := p.parser(input, *increaseMemoryPtr)
...
func exec(input io.Reader, inputValue []int, increaseMemory bool) (string, []int, error) {
	p := program{}
	err := p.parser(input, increaseMemory)
