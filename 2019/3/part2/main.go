...
func (p *Panel) shortersSteps() int {
	steps := 0

	for _, point := range p.intersectionPoints {
		if steps == 0 || point.steps < steps {
			steps = point.steps
		}
	}
	return steps
	...
	for _, wireOnePoint := range wireOnePoints.points {
		...
		for _, wireTwoPoint := range wireTwoPoints.points {
			...
			if wireOnePoint.x == wireTwoPoint.x && wireOnePoint.y == wireTwoPoint.y {
				p.intersectionPoints = append(p.intersectionPoints, point{wireTwoPoint.x, wireOnePoint.y, wireOnePoint.steps + wireTwoPoint.steps})
				break
			}
		}
	}
...
func main() {
	...
	stepsPtr := flag.Bool("steps", false, "Return the shorters steps")
