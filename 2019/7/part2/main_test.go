...
func TestExecSequence(t *testing.T) {
	for _, test := range []struct {
		...
		},
		{
			...
			"3,26,1001,26,-4,26,3,27,1002,27,2,27,1,27,26,27,4,27,1001,28,-1,28,1005,28,6,99,1,139629729,0",
			[]int{9, 8, 7, 6, 5},
			[]int{129, 4257, 136353, 4363425, 139629729},
		},
		{
			"3,52,1001,52,-5,52,3,53,1,52,56,54,1007,54,5,55,1005,55,26,1001,54,-5,54,1105,1,12,1,53,54,53,1008,54,0,55,1001,55,1,55,2,53,55,53,4,53,1001,56,-1,56,1005,56,6,99,0,0,0,0,10",
			"3,52,1001,52,-5,52,3,53,1,52,56,54,1007,54,5,55,1005,55,26,1001,54,-5,54,1105,1,12,1,53,54,53,1008,54,0,55,1001,55,1,55,2,53,55,53,4,53,1001,56,-1,56,1005,56,6,99,1,18216,2,1,0",
			[]int{9, 7, 8, 5, 6},
			[]int{19, 58, 128, 271, 552, 1123, 2266, 4544, 9103, 18216},
		},
	} {
		var b bytes.Buffer
		b.WriteString(test.input)
		...
		if len(output) != len(test.output) {
			t.Fatalf("output values len (%v) is equal to wanted output (%v)", output, test.output)
		}
		for i, v := range test.output {
			if v != output[i] {
				t.Errorf("Got %d output but expected: %d", output[i], v)
