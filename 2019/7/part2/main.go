...
func (p *program) eval() error {
	...
	for p.hasNext() {
		...
		case saveInstruction:
			if len(p.input) < p.currentInputIndex+1 {
				p.position--
				return nil
			}

			position := p.next()
			err := p.store(p.input[p.currentInputIndex], position)
			if err != nil {
				return err
			}
			p.currentInputIndex++
...
func generateMaxSequence(p *program, inputValue []int) (string, []int, error) {
	...
	for c := 1; c < 120; c++ {
		...
		lastOutput := len(o) - 1
		if o[lastOutput] > maxOutput {
			maxOutput = o[lastOutput]
			result = r
			output = o
		}
	}
...
func execSequence(p *program, inputValue []int) (string, []int, error) {
	...
	amps := make([]*program, len(inputValue))
	for i := 0; i < len(inputValue); i++ {
		opsCodes := make([]int, len(p.initialState))
		copy(opsCodes, p.initialState)
		amps[i] = &program{
			input:    []int{inputValue[i]},
			opsCodes: opsCodes,
		}
	}

	loopRun := 0
	lastAmpIndex := len(amps) - 1
	currentAmpIndex := 0
	currentInputValue := 0

	for amps[lastAmpIndex].currentOpsCode != haltInstruction {
		currentAmpProgram := amps[currentAmpIndex]
		currentAmpProgram.input = append(currentAmpProgram.input, currentInputValue)

		err := currentAmpProgram.eval()
		if err != nil {
			return "", []int{}, err
		}

		currentInputValue = currentAmpProgram.output[len(currentAmpProgram.output)-1]
		if currentAmpIndex == lastAmpIndex {
			currentAmpIndex = 0
			loopRun++
			continue
		}
	...
	return amps[lastAmpIndex].String(), amps[lastAmpIndex].output, nil
}
